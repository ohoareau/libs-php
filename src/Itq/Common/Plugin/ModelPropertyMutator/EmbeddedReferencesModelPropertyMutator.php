 ...
 */
class EmbeddedReferencesModelPropertyMutator extends Base\AbstractModelPropertyMutator
{
    ...
    {
        return true === isset($m['embeddedReferences'][$k]);
    }
    /**
     * @param ModelInterface $doc
     * @param string         $k
    ...
    public function mutate($doc, $k, $v, array &$m, array &$data, Closure $objectMutator, array $options = [])
    {
        ...
        return $objectMutator(
            ...
            $this->createModelInstance(
                [
                    'model' => $this->getMetaDataService()->getModelClassForId($m['embeddedReferences'][$k]['localType']),
                ]
            ),
            $options
        );
